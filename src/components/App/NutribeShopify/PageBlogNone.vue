<!--
  ...
  Not use this file.
  ce fihcier a été regrouper ailleur.
-->
...
<template>
  <div>
    ...
    <div v-show="show_button">
      <div class="d-flex">
        <div class="py-2 pr-2 d-inline-block">
          <button
            class="btn btn-outline-primary"
            @click="open_modal"
            data-action="gestion-blocs"
          >
            <i class="fas fa-sliders-h"></i> Gestion des categories
          </button>
        </div>
      </div>
    </div>
...
export default {
  ...
  mounted() {
     ...
     */
    //this.VueGestionBlocs = this.$refs.modal_simple.$refs.cpts_modal_simple;
  },
  components: {
  ...
  methods: {
    open_modal(event) {
      ...
      if (action == "gestion-blocs") {
        this.template_modal_body = GestionBlocs;
        $("#" + self.formated_id_modal).modal({});
        $("#" + self.formated_id_modal).on("shown.bs.modal", function() {
          $("#" + self.formated_id_modal).off("shown.bs.modal");
          if (self.id_blog != "" && !self.blog_category.length) {
            self.VueGestionBlocs =
              self.$refs.modal_simple.$refs.cpts_modal_simple;
            self.VueGestionBlocs.set_loading_blocs(true);
            self.getBlog();
          }
        });
      } else {
        this.template_modal_body = "";
      }
    },
    getBlog() {
