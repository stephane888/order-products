 ...
 */
import table_edit_datas from "/siteweb/PluginsModules/stephane888/wbu-components/src/components/Tableau/Elements/EditRow.vue";
...
/**
 *  On doit rendre ce composant externe y compris sa css.
 */
const table_template_tableau_action = {
  name: "TableTemplateTableauAction",
  props: {
    ligne: {
      type: [String, Boolean, Object, Array]
    },
  ...
  data() {
    return {
      backgroundColor: "rgb(210, 217, 224)"
    };
  },
  template: `<div class="setting-action" >
    <span class="button" @click="edit_ligne"><i class="far fa-edit"></i></span>
  </div>`,
  methods: {
    edit_ligne(event) {
      var self = this;
      ...
      /**
       * On recupere la position du premier element (tr)
       */
      $(event.target)
        .closest("tr")
        .map(function() {
          var offsetParent = $(this).offset();
          ...
          $(".setting-edit")
            ...
            .offset(offsetParent);
          /**
           * On recupere les dimenssions de chacun des cellules.
           */
          var offsets = [];
          $(this)
            .find("td")
          ...
          self.$emit("ev_template_tableau_action", {
            ligne: self.ligne,
            index: self.index,
            offsets: offsets
          });
...
export default {
  ...
  data: function() {
    return {
      ...
      table_template_tableau_action: table_template_tableau_action,
      table_datas_ligne_edit: {},
      // Contient le formulaire d'edition des données.
      table_template_edit: table_edit_datas,
    ...
    ev_template_edit(datas) {
      ...
      if (datas.action == "close") {
        ...
        if (datas.params.action == "delete") {
          this.closeTemplateEdit();
          /*
          if (this.blog_category.length > 1) {
            this.blog_category.splice(datas.forms.index, 1);
          } else {
            console.log("données du tableau vide");
            this.blog_category = [];
          }
          /**/
          this.blog_category.splice(datas.forms.index, 1);
    ...
    ev_ajax_success(datas) {
      ...
      if (
        ...
        var reseulKeySearch = async function() {
           ...
           */
          var result = await self.getMetafieldByKey("blog_category");
